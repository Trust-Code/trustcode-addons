...
class LibraryBooks(models.Model):
    ...
    publishing_company_id = fields.Many2one(
        'res.partner', string='Publishing Company', required='1')
    original_publishing_company = fields.Char(
        'Publishing Company(Orig. Name)', size=150,
        help="Publishing company's original name")
    edition = fields.Char(
        'Edition', help="Edition Number", size=2, required='1')
    release_year = fields.Integer('Release Year')
    chapters = fields.Integer('Chapters')
    pages = fields.Integer('Pages')
    original_publication_id = fields.Many2one(
        'library.books.publication.type', string='Original Pub.')
    ...
    access_type_id = fields.Many2one(
        'library.books.access.type', string='Access')
    series_collection_id = fields.Many2one(
        'library.books.series.collection', string='Series/Colection')
    ...
    feedback_type_id = fields.Many2one(
        'library.books.feedback.type', string='Feedback Type')
    files_format_id = fields.Many2one(
        'library.books.files.format', string='Files Format')
    has_printed = fields.Boolean('Has Printed')
    is_digitized = fields.Boolean('Digitized')
    is_backoffice = fields.Boolean('Backoffice')
    pdf_split = fields.Selection(
        [('not_applicable', 'Not Applicable'), ('ok', 'OK'),
         ('produce', 'Produce')], string='PDF Split')
    cover_pages = fields.Selection(
        [('ok', 'OK'), ('produce', 'Produce')], string='Cover Pages')
    ...
    value_usd_production = fields.Float(
        'USD', help="Production Cost(USD - United States Dollar)")
    value_br_production = fields.Float(
        'R$', help="Production Cost (Real - Brazilian Coin)")
    ...
    responsible_provider = fields.Many2one(
        'res.partner', string="Responsible Provider for Marking")
