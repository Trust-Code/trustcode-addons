...
class SaleOrderLineItemRestore(models.TransientModel):
    ...
    product_uom_qty = fields.Float(related='order_line_id.product_uom_qty', readonly=True)
    price_unit = fields.Float(related='order_line_id.price_unit', readonly=True)
    discount = fields.Float(related='order_line_id.discount', readonly=True)
    currency_id = fields.Many2one(related='order_line_id.currency_id', readonly=True)
    price_subtotal = fields.Monetary(related='order_line_id.price_subtotal', readonly=True)
...
class RestoreItemsWizard(models.TransientModel):
    ...
    def onchange_order_id(self):
        self.line_ids = [(0, False, {'order_line_id': x.id}) for x in self.order_id.order_line]
      
    def action_restore_all(self):
        if self.order_id.current_revision_id.state != 'draft':
            raise UserError('Você pode restaurar apenas quando em provisório. Crie uma nova versão antes!')
        self.order_id.current_revision_id.order_line.unlink()
        for item in self.line_ids:
            item.order_line_id.copy({'order_id': self.order_id.current_revision_id.id})
