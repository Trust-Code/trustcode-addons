...
class ProductImageImportWizard(models.TransientModel):
    ...
    product_model = fields.Selection(
        [('1', 'Product Template'),
         ('2', 'Product Variants')], string="Product Model")
    ...
    def import_file(self):
        ...
        for row in reader:
            ...
            if "http://" in image_path or "https://" in image_path:
                try:
                    link = urlopen(image_path).read()
                    image_base64 = base64.encodestring(link)
                except Exception:
                    raise Warning("Please provide correct URL for product '%s'\
                        or check your image size.!" % product)
            else:
                try:
                    with open(image_path, 'rb') as image:
                        image_base64 = base64.b64encode(image.read())
                except IOError:
                    raise Warning("Could not find the image '%s' - please make\
                        sure it is accessible to this script" %
                                  product)
            ...
            if product_id:
                ...
                raise Warning("Could not find the product '%s'"
                              % product)
